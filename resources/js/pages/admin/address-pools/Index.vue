...
const toggleNode = (nodeId: number) => {
    const idx = poolFormData.value.node_ids.indexOf(nodeId);
    if (idx > -1) {
        poolFormData.value.node_ids.splice(idx, 1);
    } else {
                    ...
                    <table class="table">
                        ...
                        <tbody>
                            ...
                            >
                                ...
                                <td class="text-center">{{ pool.addresses_count || 0 }}</td>
                    ...
                    <table class="table">
                        ...
                        <tbody>
                            <tr v-for="addr in filteredAddresses()" :key="addr.id">
                                <td class="font-mono text-white">{{ addr.address }}</td>
                                <td>{{ addr.cidr }}</td>
                ...
                <div class="card relative z-10 w-full max-w-md">
                    ...
                    <form @submit.prevent="poolMutation.mutate()" class="card-body space-y-4">
                        ...
                        <div v-if="nodes?.length">
                            ...
                            <div class="space-y-1 max-h-40 overflow-y-auto border border-secondary-700 rounded p-2">
                                <label 
                                    v-for="node in nodes" 
                                    :key="node.id" 
                                ...
                                >
                                    <input 
                                        type="checkbox" 
                                        :checked="poolFormData.node_ids.includes(node.id)"
